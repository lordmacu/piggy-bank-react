...
export default function Home() {
  ...
  const url = new Web3.providers.HttpProvider(
    'https://ropsten.infura.io/v3/4c3357c4588344919337836ff9cd1934'
  );
  let w3 = new Web3(url);
  ...
  const depositValue = async (value) => {
   ...
   // const accounts = await window.ethereum.request({ method: 'eth_requestAccounts' });
    ...
    contract.methods.deposit(parseInt(new Date().getTime() / 1000)).request({ from: accountsFromMetaMask["result"][0], value: w3.utils.toWei(value, 'ether'), gas: 3000000 },
